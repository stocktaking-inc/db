...
BEGIN
    ...
    WHERE item_id = NEW.item_id AND warehouse_id = (
        SELECT warehouse_id FROM good WHERE id = (
            SELECT good_id FROM inventory_items WHERE item_id = NEW.item_id LIMIT 1
        )
    )
    LIMIT 1;
    ...
    VALUES (NEW.item_id, NEW.quantity, 'shipment', (
        SELECT warehouse_id FROM good WHERE id = (
            SELECT good_id FROM inventory_items WHERE item_id = NEW.item_id LIMIT 1
        )
    ), CURRENT_TIMESTAMP);
